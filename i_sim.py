...
def cosine_sim(matrix, top_k=20, self_loop=False, verbose=-1):
    ...
    if config['time'] == 1:
        filtered_similarity_matrix_csr = cosine_sim_time(matrix, binary_matrix=None, alpha=config['alpha'], decay_mode='both', decay_rate=config['t_decay'], beta=1.0, top_k=top_k, self_loop=self_loop, verbose=verbose)
    else:
        filtered_similarity_matrix_csr = cosine_sim_binary(matrix, top_k, self_loop, verbose)
    
    return filtered_similarity_matrix_csr   
    
def cosine_sim_binary(matrix, top_k=20, self_loop=False, verbose=-1):
...
def cosine_sim_time(matrix, binary_matrix=None, alpha=0.5, decay_mode='both', decay_rate=0.01, beta=1.0, top_k=20, self_loop=False, verbose=-1):
    """
